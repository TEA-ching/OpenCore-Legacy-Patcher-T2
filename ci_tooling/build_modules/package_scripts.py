...
class ZSHFunctions:
    ...
    def generate_standard_pkg_parameters(self) -> str:
        ...
        _script += "pathToTargetLocation=\"$2\"  # ex. '/', '/Applications', etc\n"
        _script += "pathToTargetVolume=\"$3\"    # ex. '/', '/Volumes/MyVolume'\n"
    ...
    def generate_script_remove_file(self) -> str:
        """
        ZSH function to remove files safely with explicit path boundaries
        """
        _script = ""
        _script += "function _removeFile() {\n"
        _script += "    local file=\"$1\"\n\n"
        _script += "    if [[ -z \"$file\" ]]; then\n"
        _script += "        return\n"
        _script += "    fi\n\n"
    ...
    def generate_set_suid_bit(self) -> str:
        """
        ZSH function to set SUID bit safely.
        Enforces strict file verification, avoiding global bundle pollution.
        """
        _script = ""
        _script += "function _setSUIDBit() {\n"
        _script += "    local binaryPath=\"$1\"\n\n"
        _script += "    if [[ -f \"$binaryPath\" ]]; then\n"
        _script += "        echo \"Setting SUID bit on binary: $binaryPath\"\n"
        _script += "        /usr/sbin/chown root \"$binaryPath\"\n"
        _script += "        /bin/chmod 4755 \"$binaryPath\"\n"
        _script += "    else\n"
        _script += "        echo \"Error: SUID target is missing or not a standard binary file: $binaryPath\"\n"
        _script += "    fi\n"
        _script += "}\n"
    ...
    def generate_create_alias(self) -> str:
        ...
        _script += "    if [[ -e \"$aliasPath\" || -L \"$aliasPath\" ]]; then\n"
        _script += "        echo \"Scrubbing old file asset node at destination target path: $aliasPath\"\n"
    ...
    def generate_fix_settings_file_permission(self) -> str:
        ...
        _script += "        /bin/chmod 600 \"$settingsPath\"\n"  # 600 is significantly safer than global 666 read/write
        _script += "    fi\n"
        _script += "}\n"
    ...
    def generate_clean_launch_service(self) -> str:
        """
        ZSH function to clean Launch Service using native shell arrays
        rather than dynamic string output execution tracking loops.
        """
        _script = ""
        ...
        _script += "            continue\n"
        _script += "        fi\n\n"
        # Uses standard ZSH glob expansion arrays completely immune to character splitting or injection
        _script += "        local serviceFiles=(\"$launchServiceVariant\"/*\"$domain\"*(N))\n"
        _script += "        for launchServicePath in \"${serviceFiles[@]}\"; do\n"
        _script += "            if [[ -e \"$launchServicePath\" || -L \"$launchServicePath\" ]]; then\n"
...
class GenerateScripts:
    ...
    def _generate_uninstall_script(self) -> str:
        ...
        _script += self.__generate_shebang()
        _script += self._generate_header_bar()
        _script += "# OpenCore Legacy Patcher Uninstall Script\n"
        _script += self._generate_header_bar()
        _script += "\n\n"
        _script += self.zsh_functions.generate_standard_pkg_parameters()
        _script += "\n\n"
